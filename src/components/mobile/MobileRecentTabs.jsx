...
export default function MobileRecentTabs({ isOpen, onClose }) {
  ...
  const closeAllApps = useDesktopStore(s => s.closeAllApps)
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 bg-black/40 backdrop-blur-md flex flex-col p-6 overflow-y-auto pointer-events-auto"
    >
      <div className="flex-1 flex flex-col gap-4 mt-8">
        ...
        {windows.length === 0 && (
          ...
          <div className="mt-6 mb-16 mx-auto flex gap-3">
            <button 
              onClick={() => {
                closeAllApps()
                onClose()
              }}
              className="bg-red-500/40 active:bg-red-500/50 text-white rounded-full px-8 py-3 font-semibold backdrop-blur-md transition-colors"
            >
              Close All
            </button>
          </div>
